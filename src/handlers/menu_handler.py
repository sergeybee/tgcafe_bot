...
from src.keyboards.inline.ikb_menus import menu_cd, incr_decr_btn
...
async def msg_call_list_categories(message: Union[types.CallbackQuery, types.Message], **kwargs):
    if isinstance(message, types.Message):
        await message.answer(f"Выберите раздел:", reply_markup=ikb_categories())
...
async def callback_show_product(callback: types.CallbackQuery, category_id, item_id):
    ...
    await callback.message.answer(text, reply_markup=ikb_product(category_id, item_id))

    await callback.answer()

# async def call_increase(callback: types.CallbackQuery, count, action):
#     quantity_value = user_data.get(call.from_user.id, 0)
#         action = callback_data["action"]
#         if action == "incr":
#             user_data[call.from_user.id] = quantity_value + 1
...
async def navigate(callback: types.CallbackQuery, callback_data: dict):
    ...
    levels = {
        "1": msg_call_list_categories,  # Отдаем категории
...
def register_menu_handlers(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(navigate, incr_decr_btn.filter(action=['incr', 'decr']), state="*", is_user=True)
